...
import { getGame, waittingInLobby } from "../../services/gameServices";
...
export function Game() {
  ...
  const fetchGame = React.useCallback((gameId) => {
  ...
  switch (state.tag) {
    ...
    case 'notYourTurn':
      return <div>Waiting for the other player...
...
function renderBoard(boardSize: number, cellSize: number, grid: string[], handleIntersectionClick: (rowIndex: number, colIndex: number, size: number) => void) {
        ...
        [...Array(boardSize + 1)].map((_, colIndex) => {
          ...
          return (
            <div
              ...
              onClick={handleIntersectionClick ? () => handleIntersectionClick(rowIndex, colIndex, boardSize) : undefined}
            >
              {(tem === 'W' || tem === 'B') && (
                <span style={pieceStyle}>{tem}</span>
              )}
            </div>
          );
        })
