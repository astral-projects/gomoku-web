...
class GamesService(
     ...
     */
    // TODO:
    //  Idempotency key should be generated by the client - the client simply added it to the request?
    //  How can i add to the db in the first time? - I add it in the transaciton but the spring interceptor is availing first
    //  I need to add it in the interceptor - but then i will need to hardcode the gameId in the interceptor
    //  Need do comprehend better the expiration time and how to use it. How much time should the key live?
    //  Evaluate the point 2.6 of the memo - https://datatracker.ietf.org/doc/html/draft-idempotency-header-01#section-2.6
    //  How can i test it?
    fun makeMove(gameId: Id, userId: Id, square: Square, idempotencyKey: IdempotencyKey): GameMakeMoveResult =
        transactionManager.run { transaction ->
            val gamesRepository = transaction.gamesRepository
            val game = gamesRepository.getGameById(gameId)
            ...
            gamesRepository.getIdempotencyKeyInfo(idempotencyKey.idempotencyKey)
                ?: return@run failure(GameMakeMoveError.IdempotencyKeyNotFound)
            if (gamesRepository.isIdempotencyKeyUsed(idempotencyKey.idempotencyKey, gameId)) {
                return@run failure(GameMakeMoveError.IdempotencyKeyAlreadyUsed)
            } else {
                gamesRepository.markIdempotencyKeyAsUsed(idempotencyKey.idempotencyKey, gameId)
            }
    ...
    /**
     * Retrieves the idempotency key info and marks it used if is the first request.
     * @param idempotencyKey the idempotency key to retrieve the info.
     * @param gameId the id of the game that used the idempotency key.
     * @return the idempotency key info.
     */
    fun getIdempotencyKeyInfo(idempotencyKey: UUID, gameId: Id): IdempotencyKey? =
        transactionManager.run {
            val gamesRepository = it.gamesRepository
            val idempotencyKeyInfo = gamesRepository.getIdempotencyKeyInfo(idempotencyKey)
            // For the first time, the idempotency key is not in the database
            if (idempotencyKeyInfo == null) {
                gamesRepository.markIdempotencyKeyAsUsed(idempotencyKey, gameId)
                return@run gamesRepository.getIdempotencyKeyInfo(idempotencyKey)
            } else {
                return@run idempotencyKeyInfo
            }
        }
